...
const DisposableFoldersView: React.FC<{ initialFilter?: DisposalViewType }> = ({ initialFilter }) => {
    ...
    const [selectedFolderIds, setSelectedFolderIds] = useState<string[]>([]); // number -> string fix
    ...
    const handleSelectFolder = (id: string) => { // number -> string fix
    ...
    const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.checked) {
            setSelectedFolderIds(disposableFolders.map(f => String(f.id))); // Ensure string
        } else {
    ...
    const handleConfirmDisposal = async () => {
        try {
            ...
            const foldersToDispose = disposableFolders.filter(f => selectedFolderIds.includes(String(f.id)));
            ...
            // Local state'i güncelle - ensure comparison with string
            setDisposableFolders(prev => prev.filter(f => !selectedFolderIds.includes(String(f.id))));
            setSelectedFolderIds([]);
    ...
    return (
        <>
            ...
            {isLoading ? (
                ...
                <div className="overflow-hidden p-1 xl:p-2">
                    {disposableFolders.length > 0 && (
                        <div className="px-3 py-2 xl:px-4 xl:py-3 bg-gray-100 dark:bg-slate-700 border dark:border-gray-600 rounded-lg flex items-center mb-4">
                            <input 
                                type="checkbox" 
                            ...
                            />
                            <span className="ml-3 text-xs xl:text-sm font-semibold text-gray-700 uppercase dark:text-gray-400">
                    ...
                    <div className="space-y-2 xl:space-y-3">
                        {disposableFolders.map(folder => {
                            ...
                            return (
                                <div
                                    key={folder.id}
                                    className="p-2 xl:p-3 border rounded-lg dark:border-gray-600 bg-white dark:bg-archive-dark-panel transition-all duration-300 hover:shadow-md cursor-pointer flex items-start gap-3"
                                    onClick={() => handleSelectFolder(String(folder.id))}
                                >
                                    <input 
                                        type="checkbox" 
                                        checked={selectedFolderIds.includes(String(folder.id))} 
                                        readOnly
                                    ...
                                    <div className="flex-grow min-w-0">
                                        <div className="flex items-center gap-2 mb-1">
                                            ...
                                            <h4 className="font-semibold text-xs xl:text-sm text-gray-900 dark:text-white">{getDepartmentName(folder.departmentId)} - {folder.subject}</h4>
                                        </div>
                                        <div className="text-[10px] xl:text-xs text-gray-500 dark:text-gray-400 flex flex-wrap gap-x-2 xl:gap-x-4">
                                        ...
                                        <div className="text-[10px] xl:text-xs text-gray-500 dark:text-gray-400 mt-0.5">
...
const DisposedFoldersView: React.FC = () => {
    ...
    return (
        <div className="overflow-hidden p-1 xl:p-2">
            <p className="text-xs xl:text-sm text-gray-700 dark:text-gray-400 mb-4 transition-colors duration-300">{disposals.length} adet imha edilmiş klasör bulundu.</p>
            {disposals.length === 0 ? (
                ...
                <div className="space-y-2 xl:space-y-3">
                    {disposals.map(disposal => {
                        ...
                        return (
                            <div key={disposal.id} className="p-2 xl:p-3 border rounded-lg dark:border-gray-600 bg-white dark:bg-archive-dark-panel transition-all duration-300 hover:shadow-md">
                                <div className="flex items-center gap-2 mb-1">
                                    <Badge text={folder.category} color={folder.category === Category.Tibbi ? 'green' : 'blue'} />
                                    <h4 className="font-semibold text-xs xl:text-sm text-gray-900 dark:text-white">{getDepartmentName(folder.departmentId)} - {folder.subject}</h4>
                                </div>
