...
export const LocationSelector: React.FC<LocationSelectorProps> = ({ location, onLocationChange, folderType }) => {
  ...
  // Defensive: storageStructure array'leri undefined kontrolü
  const kompaktUnits = storageStructure?.kompakt || [];
  const standUnits = storageStructure?.stand || [];
  ...
  const faces = useMemo(
    () => location.unit ? kompaktUnits.find(u => u.unit === location.unit)?.faces : [],
    [location.unit, kompaktUnits]
  );
  ...
  const shelvesForStand = useMemo(() => {
    if (location.stand) {
      return standUnits.find(s => s.stand === location.stand)?.shelves || [];
    }
    return [];
  }, [location.stand, standUnits]);
  ...
  const getOccupancyAlert = () => {
    ...
    if (remainingSpace < folderWidth) {
      return (
        <div className="p-2 bg-red-100 dark:bg-red-900/50 text-red-800 dark:text-red-300 rounded-lg text-xs mt-1 transition-colors duration-300">
    ...
    return (
      <div className="p-2 bg-yellow-100 dark:bg-yellow-900/50 text-yellow-800 dark:text-yellow-300 rounded-lg text-xs mt-1 transition-colors duration-300">
