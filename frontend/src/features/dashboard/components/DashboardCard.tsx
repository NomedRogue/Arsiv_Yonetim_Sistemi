...
export const DashboardCard: React.FC<DashboardCardProps> = ({ title, value, icon: Icon, className }) => {
  ...
  return (
    ...
    >
      <div className="dashboard-card-content p-3">
        <div className="flex items-center justify-between">
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium text-gray-600 dark:text-gray-300 truncate">{title}</p>
            <p className="text-lg font-bold text-gray-900 dark:text-white mt-1 leading-tight">{value}</p>
          </div>
          <div 
            className="icon-container flex-shrink-0 ml-2 transition-colors duration-200"
            style={{ color: iconColor }}
          >
            <Icon className="dashboard-icon w-6 h-6" />
          </div>
        </div>
      </div>
    </div>
