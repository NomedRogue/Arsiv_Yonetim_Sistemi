...
import { getDepartmentColor, getTreemapColors, THEME_COLORS } from '@/lib/theme';
...
export const CustomizedTreemapContent = (props: any) => {
  ...
  if (depth === 1) {
    const colors = getTreemapColors(theme, category || name, true);
    return (
      <g>
        <rect
          x={x}
        ...
        />
        <text x={x + 8} y={y + 20} fill={colors.text} fontSize="1rem" fontWeight="bold">
          {name}
        </text>
      </g>
    );
  ...
  if (depth === 2) {
    // Use centralized theme colors with percentage for proper color intensity
    const fillColor = getDepartmentColor(theme, category || '', index, percentage);
    const colors = getTreemapColors(theme, category || '', false);
